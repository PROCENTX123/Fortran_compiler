...
class NonTerminal(Symbol):
    ...
    def __ior__(self, other):
        def is_callable(obj): return hasattr(obj, '__call__')
        def is_fold(obj): return is_callable(obj) or isinstance(obj, ExAction)
...
class ParsingTable:
    ...
    def __setup_from_grammar(self, gr):
        ...
        self.goto = tuple({x: None for x in self.nonterms}
                          for i in range(self.n_states))
        self.action = tuple({x: set() for x in self.terminals}
                            for i in range(self.n_states))
        ...
        for symbol in (self.terminals + self.nonterms):
            for state_id in range(self.n_states):
                ...
                if len(next_state) == 0:
                    continue
                next_state_id = id_from_core[drop_itemset_lookaheads(
                    next_state)]
            ...
            for item, next_symbol in self.__ccol[state_id]:
                ...
                if dot < len(pbody):
                    ...
                    if prod_index == 0:
                        ...
                        self.action[state_id][next_symbol].add(
                            Reduce(prod_index))
    ...
    def __stringify_action_entries(term, ent):
        if len(ent) > 1:
            ...
            conflict = ''
        return '\tfor terminal %s: ' % term + ', '.join(map(str, ent)) + conflict
    ...
    def stringify_state(self, state_id):
        ...
        actions = [(t, e)
                   for t, e in self.action[state_id].items() if len(e) > 0]
        actions_str = '\n'.join(
            self.__stringify_action_entries(t, e) for t, e in actions)
        actions_str += ('\n' if len(actions_str) > 0 else '')

        gotos = [(nt, sid)
                 for nt, sid in self.goto[state_id].items() if sid is not None]
        gotos = sorted(gotos, key=lambda elem: elem[0].name)

        gotos_str = '\n'.join(self.__stringify_goto_entry(nt, sid)
                              for nt, sid in gotos)
        gotos_str += ('\n' if len(gotos_str) > 0 else '')

        action_goto_separator = ('\n' if len(
            actions_str) > 0 and len(gotos_str) > 0 else '')
        return state_title + items_str + actions_str + action_goto_separator + gotos_str

    def stringify(self):
        states_str = '\n'.join(self.stringify_state(i)
                               for i in range(self.n_states))
        return states_str
    ...
    def get_single_state_conflict_status(self, state_id):
        seq = [self.__get_entry_status(e)
               for t, e in self.action[state_id].items()]
...
def get_canonical_collection(gr):
    ...
    table = [{item: LrZeroItemTableEntry() for item in kstates[i]}
             for i in range(n_states)]
    table[0][(0, 0)].lookaheads.add(EOF_SYMBOL)

    for i_state_id in range(n_states):
        state_symbols = [x[1]
                         for x, y in dfa.goto.items() if x[0] == i_state_id]
            ...
            for sym in state_symbols:
                ...
                for ((prod_index, dot), next_symbol) in closure_set:
                    ...
                    if next_symbol == FREE_SYMBOL:
                        table[i_state_id][i_item].propagates_to.add(
                            (j_state_id, j_item))
                    else:
...
class Parser(object):
    ...
    def __build_first_sets(self):
        ...
        self.__first_sets = {x: frozenset(y)
                             for x, y in self.__first_sets.items()}
            ...
            match action:
                ...
                case Reduce(rule):
                    ...
                    attrs = [attr for state, coord, attr in stack[len(stack)-n:]
                             if attr != None]
                    coords = [coord for state, coord,
                              attr in stack[len(stack)-n:]]
                    if len(coords) > 0:
                        res_coord = Fragment(
                            coords[0].start, coords[-1].following)
                    else:
            ...
            for item_set in set_queue:
                ...
                for symbol in gr.symbols:
                    ...
                    if next_item_set not in seen:
                        ...
                        self.id_from_state[self.states[-1]] = next_id
                        next_id += 1
                    self.goto[(item_set_id, symbol)
                              ] = self.id_from_state[next_item_set]
